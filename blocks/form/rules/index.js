    ...
    switch (propertyName) {
      ...
      case "value":
        ...
        if (
          ...
          field
            ...
            ?.replaceWith(createOptimizedPicture(valueToSet, altText));
      ...
      case "enabled":
        ...
        if (fieldType === "radio-group" || fieldType === "checkbox-group") {
          if (readOnly === false) {
            field
              .querySelectorAll(`input[name=${id}_${name}]`)
              .forEach((el) => {
          ...
          if (readOnly === false) {
            fieldWrapper
              ...
              ?.classList.toggle("disabled", !currentValue);
          }
        } else {
      ...
      case "readOnly":
        if (fieldType === "radio-group" || fieldType === "checkbox-group") {
          ...
          fieldWrapper
            .querySelector(".rating")
            ?.classList.toggle("disabled", currentValue === true);
        } else {
...
function applyRuleEngine(htmlForm, form, captcha) {
  ...
  htmlForm.addEventListener("click", async (e) => {
    ...
    if (button) {
      // For submit button, validate and redirect directly
      if (button.type === "submit") {
        e.preventDefault();
        e.stopPropagation();
        const valid = htmlForm.checkValidity();
        if (valid) {
          window.location.href =
            "/index/form-page/forms-submission-confirmation";
          return;
        }
        // Show validation errors
        const firstInvalidEl = htmlForm.querySelector(":invalid:not(fieldset)");
        if (firstInvalidEl) {
...
async function initializeRuleEngineWorker(formDef, renderHTMLForm) {
  if (typeof Worker === "undefined") {
    ...
    const ruleEngine = await import("./model/afb-runtime.js");
    const form = ruleEngine.createFormInstance(
      { ...formDef, data },
      undefined,
      LOG_LEVEL
    );
  ...
  const myWorker = new Worker(
    `${window.hlx.codeBasePath}/blocks/form/rules/RuleEngineWorker.js`,
    { type: "module" }
  );
  ...
  return new Promise((resolve) => {
    let form, captcha, data, generateFormRendition;
...
export async function initAdaptiveForm(formDef, createForm) {
  await registerCustomFunctions(
    formDef?.properties?.customFunctionsPath || "/blocks/form/functions.js",
